            ...
            <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0">
                        ...
                        <div class="mt-0.5 truncate text-[0.7rem] text-slate-500">{{ $student->email }}</div>
                    </div>
                ...
                <div class="mt-3 flex gap-2 border-t border-slate-100 pt-3">
                    <a href="{{ route('students.edit', $student) }}" class="inline-flex h-8 flex-1 items-center justify-center gap-1 rounded-lg border border-slate-200 bg-white text-xs font-semibold text-slate-700 transition hover:border-amber-300 hover:bg-amber-50/50 hover:text-amber-700">
                        <x-icon name="heroicon-o-pencil-square" class="h-3.5 w-3.5 text-slate-400" />
                        <span>Edit</span>
                    </a>
                    <form method="POST" action="{{ route('students.destroy', $student) }}" data-confirm-message="Hapus siswa ini?" class="inline-flex flex-1">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="inline-flex h-8 w-full items-center justify-center gap-1 rounded-lg border border-slate-200 bg-white text-xs font-semibold text-rose-600 transition hover:border-rose-200 hover:bg-rose-50" title="Hapus Siswa">
